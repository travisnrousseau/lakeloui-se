...
# Replace WeatherLink + OpenRouter variable lines; keep the rest of the file.
tmp=$(mktemp)
grep -v '^weatherlink_api_key\s\|^weatherlink_api_secret\s\|^weatherlink_station_id\s\|^weatherlink_station_id_base\s\|^openrouter_api_key\s' "$TFVARS" > "$tmp" || true
{
  head -n 3 "$tmp"
...
echo "Updated WeatherLink and OpenRouter vars in $TFVARS from backend/.env"
